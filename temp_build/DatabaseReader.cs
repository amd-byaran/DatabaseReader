...
public static class DcPgConn
{
    ...
    /// <summary>
    /// Retrieves comprehensive report information for a given release and coverage type.
    ...
    /// - covType: Coverage type (e.g., "code_cov" or "func_cov")
    ///
    /// Returns:
    /// - Tuple containing: (releaseId, reportId, projectName)
    /// - null: If release is not found or has no reports of the specified coverage type
    ///
    /// Queries performed:
    /// 1. Get release ID from release name
    /// 2. Get the first report for that release with the specified coverage type
    /// </summary>
    public static (int releaseId, int reportId, string projectName)? GetReportInfo(string releaseName, string covType)
    {
        ...
        string query = @"
            ...
            WHERE release_ref = $1 AND mr.cov_type_ref = $2
            ORDER BY cmr.id DESC
            LIMIT 1
        ";

        var result2 = SelectFirst(query, releaseId, CoverageType);
    ...
    /// </summary>
    public static List<(int releaseId, int reportId, string projectName, string reportName)> GetAllReportsForRelease(string releaseName, string covType)
    {
    ...
    {
        //proj/videoip/web/merged_reports/dcn6_0/dcn6_0/code_cov/dcn_core/individual/8222907
